    ...
    return (
        <fieldset>
            ...
            <div className="PriceRangeInputs">
                ...
                />
                <span style={{marginTop: '20px', marginRight: '5px'}}>&nbsp;–&nbsp;</span>
                <input
                    type="text"
                    id="upperPrice"
                    placeholder="0.0"
    ...
    const addLiquidity = async (e) => {
        ...
        try {
            ...
            if (allowance0.lt(amount0Desired)) {
                ...
                const tx = await token0.approve(config.managerAddress, ethers.constants.MaxUint256);
                await tx.wait();
            }

            if (allowance1.lt(amount1Desired)) {
                console.log('Approving token1...');
                const tx = await token1.approve(config.managerAddress, ethers.constants.MaxUint256);
                await tx.wait();
            }

            console.log('Minting...');
            const tx = await manager.mint(mintParams, {gasLimit: 1000000});
            await tx.wait();
        ...
        <section className="LiquidityWrapper">
            <form className="LiquidityForm">
                ...
                <AmountInput
                    ...
                    token={token1Info}/>
                <button className="addLiquidity" disabled={!enabled || loading} onClick={addLiquidity}>Add liquidity
                </button>
            </form>
        </section>
    );
