...
const SwapInput = ({token, tokens, onChange, amount, setAmount, disabled, readOnly}) => {
    return (
        <fieldset className="SwapInput" disabled={disabled}>
            <input type="text" id={token + "_amount"} value={amount} onChange={(ev) => setAmount(ev.target.value)}
                   readOnly={readOnly}/>
    ...
    const swap = async (e) => {
        ...
        try {
            ...
            if (allowance.lt(amountIn)) {
                const approveTx = await token.approve(config.managerAddress, uint256Max);
                await approveTx.wait();
            }
            console.log('Swap parameters:', params);

            const gasEstimate = await manager.estimateGas.swap(params);
            console.log('Estimated gas:', gasEstimate.toString());

            const swapTx = await manager.swap(params, {gasLimit: gasEstimate.mul(2)}); // Set gasLimit to 2x of the estimated gas
            await swapTx.wait();
            ...
            {path ?
                <form className="SwapForm">
                    <SwapInput
                        ...
                        tokens={tokens}/>
                    <ChangeDirectionButton zeroForOne={zeroForOne} onClick={toggleDirection}
                                           disabled={!enabled || loading}/>
                    <SwapInput
